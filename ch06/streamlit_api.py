...
def add_employee(name, dob, paygrade):
    data = {"name": name, "date_of_birth": dob, "paygrade_id": paygrade}
    response = requests.post(f"{url}/employee", json=data)
...
dob = str(
    form.date_input("DOB", min_value=datetime.datetime(year=1920, day=1, month=1))
)
